...
var SimpleExcel = (function () {
	var table, parent, controls,
	    ...
	    blankColumn = document.createElement('TD'),
	    ExlActions = {
	        create: {
	...
	/**
	 * @param {string} selector - selector to the point of attachment
	 * @constructor
	 */
	function construct(selector) {
		...
		blankColumn.innerHTML = '<input type="text">';
	...
	function addCol() {
		...
		for (i = 0, len = table.rows.length; i < len; i++) {
		    table.rows[i].appendChild(blankColumn.cloneNode(true));
		}
		blankRow.appendChild(blankColumn.cloneNode(true));
	}
	/** @public */
	function addRow() {
	...
	function controlsHandler(event) {
	    var e = event || window.event,
	        target = e.target || e.srcElement;
	    try {
	        if (target.dataset.excelCmd != null) {
